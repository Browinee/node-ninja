...
const FIN_OPCODE_PAYLOAD_LENGTH = 2;
function encodeMessage(opcode, payload) {
  // NOTE: here only handle payload.length < 126
  let bufferData = Buffer.alloc(payload.length + FIN_OPCODE_PAYLOAD_LENGTH);
...
class MyWebsocket extends EventEmitter {
  ...
  send(data) {
    ...
    if (Buffer.isBuffer(data)) {
      ...
      console.error("not support data type");
    }
